...
import { Platform, StyleSheet, View, SafeAreaView, KeyboardAvoidingView, Animated} from 'react-native';
...
class GiftedChat extends React.Component {
    constructor(props) {
        ...
        this.onGestureEvent = Animated.event([{ nativeEvent: { translationX: this.moveX } }], { useNativeDriver: true })
    ...
    render() {
        if (this.state.isInitialized === true) {
            ...
            return (
                <PanGestureHandler onGestureEvent={this.onGestureEvent} onEnded={() => this.moveX.setValue(0)} activeOffsetX={-20}>
                    <Animated.View style={[styles.safeArea]}>
                        <ActionSheetProvider ref={(component) => (this._actionSheetRef = component)}>
                            <View style={[styles.container, {width : widthPercentageToDP("130%")}]} onLayout={this.onMainViewLayout}>
                            <Animated.View style={{
                                transform: [
                                { 
                                    translateX: this.moveX.interpolate({ inputRange: [0, 40, 60, 80], outputRange: [0, 15, 17, 19] }) 
                                }
                            ],
                            }}>
                            {this.renderMessages()}
